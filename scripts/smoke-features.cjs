...
const verifiedScreenshot = context.normalizeTextReportImport(fs.readFileSync(path.join(root, "scripts/fixtures/current-month-verified.csv"), "utf8"));
const verifiedRows = verifiedScreenshot.stores.filter((item) => !item.missingReportRow);
assert.equal(verifiedRows.length, 20);
assert.equal(verifiedRows.find((item) => item.storeNumber === "1139").metrics[0].mtd, 29);
assert.equal(verifiedRows.find((item) => item.storeNumber === "519").metrics[4].mtd, 200);
assert.equal(compactMetrics[0].mtd, 45, "Post PSPD is converted only when a total is unavailable");
assert.equal(compactMetrics[1].mtd, 34);
assert.equal(compactMetrics[2].mtd, 30);
assert.equal(compactMetrics[3].mtd, 4.76);
assert.equal(compactMetrics[4].mtd, 1616);
assert.equal(context.performanceMetricsFromRecord({ ...compactRow, postacts: "49" })[0].mtd, 49);
const priorState = { stores: [structuredClone(store)], settings: { reportAsOf: "2026-08-31", importPeriod: "previous", mtdMultiplier: 31 }, history: [] };
const lastMonthState = context.finalizeImportedState({ stores: [{ ...structuredClone(store), metrics: compactMetrics }] }, priorState);
assert.equal(lastMonthState.stores[0].metrics[0].mtd, store.metrics[0].mtd, "last month must not overwrite MTD");
lastMonthState.settings = { reportAsOf: "2026-09-08", importPeriod: "current", mtdMultiplier: 8 };
const twoMonths = context.finalizeImportedState({ stores: [{ ...structuredClone(store), metrics: compactMetrics }] }, lastMonthState);
assert.equal(Object.keys(twoMonths.stores[0].monthlyReports).length, 2);
assert.match(context.buildMonthlyComparison(twoMonths.stores[0]), /August 2026/);
assert.match(context.buildMonthlyComparison(twoMonths.stores[0]), /projected month-end/);
assert.match(context.buildRichEmailHtml(twoMonths.stores[0]), /Monthly Performance/);
assert.match(context.buildPolishedEmail(twoMonths.stores[0]), /September 2026/);
vm.runInContext(`state.settings = globalThis.savedImportSettings;`, context);
